...
本项目移植自 [listenzz/HBDNavigationBar](https://github.com/listenzz/HBDNavigationBar)，并针对 Swift、现代 UIKit 和 iOS 18+ 的导航栏实现进行了适配。

## 介绍

`UINavigationBar` 的背景、阴影、标题和按钮样式通常由当前页面即时覆盖。当页面发生 push、pop 或交互式侧滑返回时，如果前后页面的配置不同，系统默认行为容易出现背景突然切换、阴影闪烁或标题颜色跳变。

`MonsterNavigationBar` 将导航栏的背景与内容拆开管理，在转场过程中分别计算并同步前后页面的状态，使导航栏能够连续地过渡到目标页面。它适合需要自定义导航栏样式，同时又希望保留系统导航控制器转场体验的应用。

本库主要处理以下场景：

### 阴影显示与隐藏

前后页面可以分别控制导航栏底部阴影，交互式返回过程中阴影会跟随当前页面状态平滑切换。

### 导航栏显示与隐藏

`monsterBarHidden` 只隐藏导航栏内容和背景，不从视图层级移除导航栏，因此导航栏在有无之间也可以连续过渡。

### 导航栏透明度随滚动变化

滚动页面时动态修改 `monsterBarAlpha`，即可实现导航栏从透明到不透明的渐变效果，并同步处理页面布局和标题颜色。

### 不同页面使用不同背景

每个页面都可以单独设置背景颜色或背景图片。push、pop 和侧滑返回时，颜色、图片、透明度、标题属性及按钮颜色会统一参与转场。

### 系统风格与扩展能力

组件始终保持导航栏的系统转场能力，同时提供返回拦截、全屏返回、TabBar 联动、导航栈重定向和 iOS 26 Liquid Glass 按钮样式等扩展 API。
